...
这里着重强调一个异或的性质，异或有结合律交换律且 $\forall x, x \oplus x = 0$，所以说，异或有着很好的性质，这些性质使异或构成一个阿贝尔群。对于任意阿贝尔群，我们可以使用前缀和这一有效的方法来维护区间运算。因此，我们考虑使用前缀和进行解题。

题目要求当 $x = l, y = r$ 时，$f(x, y) = 0$，也就是说 $f(x, 0) \oplus f(y - 1, 0) = 0$，显然这说明 $f(x, 0) = f(y - 1, 0)$。

那么我们同理可以推出，$x \ne l \lor y \ne r$ 时，$f(x, y) \ne 0$，证明 $\forall x \ne r, l - 1$，$f(x, 0)$ 唯一。我们有若干种方式进行构造，这里我们直接选择使用正整数。具体的过程留给代码，需要注意的是，题目要求构造正整数序列，而这样构造方式有可能会产生 0，那么我们还有一种构造方式就是当算出来的数为 0 的时候，将前缀和增 1，很容易看出此时算出数不可能为 0。
